...
const locales = {
  de: {
    ...
    quality: "/de/qualitaetskontrolle/",
    compliance: "/de/lebensmittelkontakt-konformitaet/",
    nav: [
  ...
  fr: {
    ...
    quality: "/fr/controle-qualite/",
    compliance: "/fr/conformite-contact-alimentaire/",
    nav: [
...
function form(locale, sample = false, product = "", productLabel = "") {
  const de = locale === "de";
  const behavior = product
    ? ` data-inquiry-form data-form-type="${sample ? "sample" : "quote"}"`
    : "";
  const labels = de
    ? [
        "Produkt",
  ...
  return `<form class="b2b-inquiry-form"${behavior} method="post" action="/api/inquiry" data-product="${esc(product)}"><input type="hidden" name="locale" value="${locale}"><input type="hidden" name="requestType" value="${sample ? "sample" : "quote"}">${product ? `<input type="hidden" name="productSlug" value="${esc(product)}">` : ""}${labels.map((x, i) => `<label>${x}${[0, 3, 10, 12, 13].includes(i) ? " *" : ""}${i === 14 ? `<textarea name="${names[i]}" rows="5"></textarea>` : `<input ${i === 12 ? 'type="email"' : i === 13 ? 'type="tel"' : ""} name="${names[i]}" ${i === 0 && product ? `value="${esc(productLabel || product)}"` : ""} ${[0, 3, 10, 12, 13].includes(i) ? "required" : ""}>`}</label>`).join("")}${product ? '<p class="form-note" data-inquiry-status></p>' : ""}<input class="hp-field" name="website" tabindex="-1"><button type="submit" data-submit-text="${sample ? (de ? "Muster anfordern" : "Demander des échantillons") : de ? "Anfrage senden" : "Envoyer la demande"}">${sample ? (de ? "Muster anfordern" : "Demander des échantillons") : de ? "Anfrage senden" : "Envoyer la demande"}</button></form>`;
}
function cta(locale, text, product = "") {
  const c = locales[locale];
  const productAttr = product ? ` data-product="${esc(product)}"` : "";
  return `<section class="cta-band"><div><p class="eyebrow">LANGMAI OEM / ODM</p><h2>${text}</h2></div><div class="hero-actions"><a class="button primary" href="${c.contact}" data-cta-type="quote"${productAttr}>${c.quote}</a><a class="button secondary" href="${c.sample}" data-cta-type="sample"${productAttr}>${c.sampleLabel}</a></div></section>`;
}
function home(locale) {
  ...
  const content = `<section class="home-hero"><div><p class="eyebrow">${de ? "Hersteller seit 2006 · OEM / ODM" : "Fabricant depuis 2006 · OEM / ODM"}</p><h1>${de ? "Lebensmittelechte Papierverpackungen für Bäckerei, Industrie und Gastronomie" : "Emballages alimentaires en papier pour la boulangerie, la pâtisserie et la restauration"}</h1><p>${desc}</p><div class="hero-actions"><a class="button primary" href="${c.products}">${de ? "Produktkatalog ansehen" : "Découvrir nos produits"}</a><a class="button secondary" href="${c.sample}">${c.sampleLabel}</a></div></div><img src="/assets/${de ? "blog-baking-paper-quality-control.webp" : "ai-cake-cups-premium.jpg"}" alt="${de ? "Backförmchen und Backpapier" : "Caissettes de cuisson professionnelles"}"></section><section class="trust-strip"><span>${de ? "Lebensmittelkontakt-Unterlagen" : "Documents de contact alimentaire"}</span><span>${de ? "Chargenrückverfolgbarkeit" : "Traçabilité des lots"}</span><span>${de ? "PFAS-freie Optionen auf Anfrage" : "Options sans PFAS sur demande"}</span><span>Private Label</span></section><section class="section"><div class="section-heading"><h2>${de ? "Papierprodukte für professionelle Beschaffung" : "Solutions papier pour les professionnels"}</h2></div><div class="product-grid">${products
    .filter(([key]) => ["baking-cups", "muffin-cups", "baking-paper", "greaseproof", "air-fryer"].includes(key))
    .map((p) => {
      const x = p[side(locale)];
      return `<article class="product-card"><img src="/assets/${p[1]}" alt="${esc(x[1])}"><div><h3>${esc(x[1])}</h3><p>${esc(x[2])}</p><a class="text-link" href="${c.products}${x[0]}/" data-cta-type="product" data-product="${p[0]}">${de ? "Produkt ansehen" : "Voir le produit"}</a></div></article>`;
    })
    .join(
      "",
...
function productPage(locale, p) {
  const de = locale === "de",
    ...
    productH1 = de
      ? `${x[1]} für professionelle Beschaffung`
      : p[0] === "air-fryer"
        ? "Papier pour air fryer destiné à l’approvisionnement professionnel"
        : `${x[1]} pour l’approvisionnement professionnel`,
    meta = de
      ? `${x[1]} für professionelle Anwendungen mit projektbezogener Auswahl von Papier, Format, Druck und Verpackung.`
      : p[0] === "air-fryer"
        ? "Papier pour air fryer destiné aux applications professionnelles, avec choix du format, du papier et du conditionnement."
        : `${x[1]} pour applications professionnelles avec choix du papier, du format, de l’impression et du conditionnement.`,
    title = de
      ? `${x[1]} | OEM-Hersteller | LANGMAI`
      : `${x[1]} | Fabricant OEM | LANGMAI`,
    faq = de
      ? [
          [
  ...
  const content = `<section class="product-hero"><div><p class="eyebrow">${de ? "Produktkategorie" : "Catégorie"}</p><h1>${esc(productH1)}</h1><p>${esc(x[2])}</p><div class="hero-actions"><a class="button primary" href="#anfrage" data-cta-type="quote" data-product="${p[0]}">${c.quote}</a><a class="button secondary" href="${c.sample}" data-cta-type="sample" data-product="${p[0]}">${c.sampleLabel}</a></div></div><img src="/assets/${p[1]}" alt="${esc(x[1])}"></section><section class="product-template section"><div class="product-main"><section class="two-col product-block"><div><h2>${de ? "Produktspezifikation" : "Spécification produit"}</h2><p>${de ? "Abmessungen, Papiergrammatur, Farbe, Druck und Packeinheit werden projektbezogen bestätigt. Temperaturangaben gelten nach Freigabe der Materialausführung." : "Dimensions, grammage, couleur, impression et conditionnement sont confirmés par projet. La température est validée pour la matière retenue."}</p></div><div><h2>${de ? "Einsatzbereiche" : "Utilisations"}</h2><ul class="check-list"><li>${de ? "Bäckerei und Lebensmittelproduktion" : "Boulangerie et production alimentaire"}</li><li>${de ? "Großhandel und Importeure" : "Grossistes et importateurs"}</li><li>Private Label</li></ul></div></section><section class="product-block"><div class="table-wrap"><table><tbody><tr><th>${de ? "Abmessungen" : "Dimensions"}</th><td>${de ? "Nach Zeichnung, Muster oder Standardformat" : "Selon plan, échantillon ou standard"}</td></tr><tr><th>${de ? "Farbe und Druck" : "Couleur et impression"}</th><td>${de ? "Unbedruckt oder individuelles Motiv" : "Neutre ou motif personnalisé"}</td></tr><tr><th>${de ? "Verpackung" : "Conditionnement"}</th><td>${de ? "Großpackung, Retail oder Private Label" : "Vrac, retail ou marque distributeur"}</td></tr><tr><th>MOQ</th><td>${de ? "Abhängig von Material und Ausführung" : "Selon matière et configuration"}</td></tr></tbody></table></div></section><section class="two-col product-block"><div><h2>${de ? "Anpassungsprozess" : "Processus de personnalisation"}</h2><ol class="process-list">${(de ? ["Anwendung und Spezifikation klären", "Material und Druck prüfen", "Muster freigeben", "Produktion und Qualitätskontrolle", "Verpackung und Versand"] : ["Définir usage et spécification", "Vérifier matière et fichiers", "Approuver l’échantillon", "Production et contrôle", "Emballage et expédition"]).map((a) => `<li>${a}</li>`).join("")}</ol></div><div id="anfrage">${form(locale, false, p[0], x[1])}</div></section><section class="faq product-block"><h2>FAQ</h2>${faq.map(([q, a]) => `<details><summary>${q}</summary><p>${a}</p></details>`).join("")}</section><section class="product-block"><h2>${de ? "Dokumente und nächste Schritte" : "Documents et prochaines étapes"}</h2><p>${de ? "Prüf- und Konformitätsunterlagen richten sich nach Produkt, Material und Zielmarkt. Senden Sie uns Ihre Spezifikation für eine belastbare Prüfung." : "Les documents de contrôle et de conformité dépendent du produit, de la matière et du marché. Envoyez votre cahier des charges pour une étude adaptée."}</p><div class="link-grid"><a href="${c.quality}">${de ? "Qualitätskontrolle ansehen" : "Voir le contrôle qualité"}</a><a href="${c.compliance}">${de ? "Lebensmittelkontakt-Unterlagen" : "Documents de conformité alimentaire"}</a><a href="${c.contact}" data-cta-type="quote" data-product="${p[0]}">${c.quote}</a><a href="${c.sample}" data-cta-type="sample" data-product="${p[0]}">${c.sampleLabel}</a></div></section></div></section>${cta(locale, de ? "Passende Muster prüfen" : "Évaluer des échantillons adaptés", p[0])}`;
  return layout(
    locale,
    route,
    "product",
...
function corePage(locale, type) {
  ...
  const meta = type === "quality"
    ? de
      ? "Prüfpunkte für Papier, Maße, Druck, Geruch, Fettbeständigkeit, Verpackung und Chargen anhand der Produktspezifikation prüfen."
      : "Contrôlez papier, dimensions, impression, odeur, résistance aux graisses, emballage et lots selon la spécification produit."
    : type === "compliance"
    ? de
      ? "Unterlagen für Lebensmittelkontakt nach Produkt, Material und Zielmarkt mit LANGMAI prüfen."
      : "Vérifiez avec LANGMAI les documents de contact alimentaire selon le produit, la matière et le marché."
    : type === "contact"
      ? de
        ? "Produkt, Maße, Menge, Druck, Zielmarkt und benötigte Unterlagen für Ihr Angebot übermitteln."
        : "Indiquez produit, dimensions, quantité, impression, marché et documents requis pour votre devis."
      : type === "sample"
        ? de
          ? "Muster für Abmessungen, Material, Farbe, Geruch, Backverhalten und Verpackung anfordern."
          : "Demandez des échantillons pour évaluer dimensions, matière, couleur, cuisson et emballage."
        : `${d[2]} ${de ? "Projektbezogene Optionen, Dokumente und nächste Schritte für professionelle Einkäufer direkt mit LANGMAI abstimmen." : "Étudiez avec LANGMAI les options, documents et prochaines étapes adaptés à votre projet professionnel."}`;
  return layout(
    locale,
    d[0],
    type,
    "",
    type === "compliance"
      ? `${de ? "Lebensmittelkontakt" : "Conformité alimentaire"} | LANGMAI`
      : `${d[1]} | LANGMAI`,
    meta,
